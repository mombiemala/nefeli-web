-- NEFELI × Luminary — companion core (Phase 1)
-- Astrology + persistent-memory schema. Owner-scoped RLS via auth.uid();
-- service-role bypasses RLS as elsewhere in NEFELI. Idempotent / safe to re-run.
-- Fashion-free: no style_profiles, no daily_guidance.style_note.
-- (monthly_guides is deferred to Phase 2.)
...
-- ── Row Level Security (owner-scoped, idempotent) ────────────
...
begin
  foreach t in array array[
    ...
    'daily_guidance','insights'
  ] loop
    execute format('drop policy if exists %1$s_owner on %1$I;', t);
    execute format(
      'create policy %1$s_owner on %1$I for all to authenticated
